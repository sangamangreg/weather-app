...
class WeatherTestCase(TestCase):
    ...
    def test_success_scenario(self):
        weather_service = OpenWeatherService( settings.CHEMONDIS_OPENWEATHER_URL, q='mumbai',
                                              appid=settings.CHEMONDIS_WEATHER_KEY, units="metric", lang='en' )
    ...
    def test_invalid_city(self):
        weather_service = OpenWeatherService( settings.CHEMONDIS_OPENWEATHER_URL, q='a',
                                              appid=settings.CHEMONDIS_WEATHER_KEY, units="metric", lang='en' )
        self.assertRaises(requests.exceptions.HTTPError, weather_service.get_data)

    def test_empty_city(self):
        weather_service = OpenWeatherService( settings.CHEMONDIS_OPENWEATHER_URL, q='',
                                              appid=settings.CHEMONDIS_WEATHER_KEY, units="metric", lang='en' )
        self.assertRaises(requests.exceptions.HTTPError, weather_service.get_data)
